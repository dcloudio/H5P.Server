...
.paragraph{
	...
	<body>
        ...
        <input type="hidden" name="pushtype" value="link"/>
<?php
...
?>
	...
	<span class="important">仅iOS平台支持，未设置token则不通过APNS通道推送离线消息</span>
				...
				<tr>
					<td>标题:</td>
					<td><input id="title" name="title" type="text" style="width:100%" placeholder="链接推送通知的标题"<?php
if ( isset($_GET['title']) ) {
...
?>/></td>
	...
	[选填]，推送消息的标题，未设置则使用默认标题。<br/>
				...
				<tr>
					<td>内容:</td>
					<td><textarea id="content" name="content" style="width:100%" rows="3" placeholder="链接推送通知的内容"><?php
...
?></textarea></td>
	...
	[必填]，推送消息的内容。<br/>
                ...
                <tr>
                    <td>链接地址：</td>
                    <td><input id="url" name="url" type="url" style="width:100%" placeholder="推送通知的链接地址"
<?php
if ( isset($_GET['url']) ) {
    echo( " value=\"{$_GET['url']}\"" );
}
?>
	...
	[必填]，推送消息的链接地址。<br/>
        ...
        <h2>Android</h2>
        <p class="paragraph">
    不管应用在线还是离线，在系统通知栏中显示消息，点击消息后调用系统浏览器打开链接地址。
        </p>
        <h2>iOS</h2>
        <p class="paragraph">
    应用在线：接收到消息后弹出提示框，点击“取消”按钮则关闭提示框，点击“确定”按钮则会调用Safari打开链接地址。
        </p>
        <p class="paragraph">
